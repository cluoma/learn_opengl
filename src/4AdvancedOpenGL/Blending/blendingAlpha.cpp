...
int main() {
    ...
    // compile shaders
    Shader lightingShader("src/4AdvancedOpenGL/Blending/colors.vert", "src/4AdvancedOpenGL/Blending/colors.frag");
    ...
    unsigned int windowTexture = loadTexture("assets/textures/blending_transparent_window.png");


    // Grass Locations
    // -------------
    vector<glm::vec3> transpWindow;
    transpWindow.push_back(glm::vec3(-1.5f, 0.0f, -0.48f));
    transpWindow.push_back(glm::vec3(1.5f, 0.0f, 0.51f));
    transpWindow.push_back(glm::vec3(0.0f, 0.0f, 0.7f));
    transpWindow.push_back(glm::vec3(-0.3f, 0.0f, -2.3f));
    transpWindow.push_back(glm::vec3(0.5f, 0.0f, -0.6f));
    ...
    while (Running)
    {
        ...
        for (auto loc : transpWindow)
        {
            float distance = glm::length(camera.Position - loc);
            sorted[distance] = loc;
        }
