...
int main() {
    ...
    // compile shaders
    Shader lightingShader("src/4AdvancedOpenGL/Framebuffers/shader.vert", "src/4AdvancedOpenGL/Framebuffers/shader.frag");
    Shader defaultShader("src/4AdvancedOpenGL/Framebuffers/defaultShader.vert", "src/4AdvancedOpenGL/Framebuffers/defaultShader.frag");
    ...
    // Texturebuffer and Renderbuffer
    // -------------
    ...
    while (Running)
    {
        ...
        while (SDL_PollEvent(&Event))
        {
            ...
            else if (Event.type == SDL_WINDOWEVENT)
            {
                if(Event.window.event == SDL_WINDOWEVENT_SIZE_CHANGED) {
                    ...
                    glViewport(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT);
                    // change texturebuffer size
                    glBindFramebuffer(GL_FRAMEBUFFER, framebuffer);
                    glBindTexture(GL_TEXTURE_2D, textureColorbuffer);
                    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, SCREEN_WIDTH, SCREEN_HEIGHT/4, 0, GL_RGB, GL_UNSIGNED_BYTE, NULL);
                    glBindTexture(GL_TEXTURE_2D, 0);
                    glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, textureColorbuffer, 0);
                    glBindRenderbuffer(GL_RENDERBUFFER, rbo);
                    glRenderbufferStorage(GL_RENDERBUFFER, GL_DEPTH24_STENCIL8, SCREEN_WIDTH, SCREEN_HEIGHT/4);
                    glBindRenderbuffer(GL_RENDERBUFFER, 0);
                    glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_DEPTH_STENCIL_ATTACHMENT, GL_RENDERBUFFER, rbo);
                    // check framebuffer completeness
                    if(glCheckFramebufferStatus(GL_FRAMEBUFFER) != GL_FRAMEBUFFER_COMPLETE)
        ...
        // second pass (the rearview mirror)
        glBindFramebuffer(GL_FRAMEBUFFER, framebuffer);
        glClearColor(0.2f, 0.3f, 0.3f, 1.0f);
        glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT); // we're not using the stencil buffer now
        glEnable(GL_DEPTH_TEST);
        ...
        projection = glm::perspective(glm::radians(camera.Zoom/2), (float)SCREEN_WIDTH / ((float)SCREEN_HEIGHT/4), 0.1f, 100.0f);
        lightingShader.setMat4("view", glm::value_ptr(view));
        lightingShader.setMat4("projection", glm::value_ptr(projection));
        // cubes
        glBindVertexArray(cubeVAO);
        ...
        // third pass, draw the rearview mirror texturebuffer
